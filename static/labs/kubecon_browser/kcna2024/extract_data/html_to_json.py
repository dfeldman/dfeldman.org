...
# If the URL contains something that looks like an ID, we use that. Otherwise
# make one up using the SHA of the URL (first 8 chars only)
def get_evtid(url):
    split_url = url.split('/')
    if len(split_url) >= 2 and split_url[0] == 'event':
        return split_url[1]
    else:
    ...
    try:
        ...
        for entry in data:
            if entry.get('content'):
                ...
                info['evtid'] = get_evtid(entry['url'])
                if info:
